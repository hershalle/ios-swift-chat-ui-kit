...
/*
 ...
 copied the nib files because CometChatSnackBoard looks for them in the main
 bundle first. See `CometChatSnackBoard` for additional nib loading options.
 */
...
extension MessageView {
    ...
    public static func viewFromNib<T: MessageView>(layout: Layout, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! CometChatSnackBoard.viewFromNib(named: layout.rawValue)
    }
    ...
    public static func viewFromNib<T: MessageView>(layout: Layout, bundle: Bundle, filesOwner: AnyObject = NSNull.init()) -> T {
        return try! CometChatSnackBoard.viewFromNib(named: layout.rawValue, bundle: bundle, filesOwner: filesOwner)
    }
}
    ...
    open func configureTheme(_ theme: Theme, iconStyle: IconStyle = .default) {
        ...
        case .info:
            if #available(iOS 13.0, *) {
                ...
                let foregroundColor = UIColor.systemBackground
...
/*
 ...
 CometChatSnackBoard does not try to be clever by adjusting the layout based on 
