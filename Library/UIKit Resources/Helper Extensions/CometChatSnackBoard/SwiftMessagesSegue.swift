//
//  CometChatSnackBoardSegue.swift
//  CometChatSnackBoard
//
...
/**
 `CometChatSnackBoardSegue` is a configurable subclass of `UIStoryboardSegue` that utilizes
 CometChatSnackBoard to present and dismiss modal view controllers. It performs these transitions by
 becoming your view controller's `transitioningDelegate` and calling SwiftMessage's `show()`
 and `hide()` under the hood.

 To use `CometChatSnackBoardSegue` with Interface Builder, control-drag a segue, then select
 "swift messages" from the Segue Type dialog. This configures a default transition. There are
 two suggested ways to further configure the transition by setting options on `CometChatSnackBoardSegue`.
 First, and recommended, you may subclass `CometChatSnackBoardSegue` and override `init(identifier:source:destination:)`.
 ...
 presenting view controller and downcast the segue to `CometChatSnackBoardSegue`.

 `CometChatSnackBoardSegue` can be used without an associated storyboard or segue by doing the following in
 the presenting view controller.

     let destinationVC = ... // make a reference to a destination view controller
     let segue = CometChatSnackBoardSegue(identifier: nil, source: self, destination: destinationVC)
 ...
 However, these parameters can be customized by initializing the view controller with a `CometChatSnackBoard.Config` that has the `CometChatSnackBoard.Config.presentationContext` set to either `.window` or `.windowScene`:

 + note: Some additional details:
 1. Your view controller's view will be embedded in a `CometChatSnackBoard.BaseView` in order to
 utilize some CometChatSnackBoard features. This view can be accessed and configured via the
 `CometChatSnackBoardSegue.messageView` property. For example, you may configure a default drop
 shadow by calling `segue.messageView.configureDropShadow()`.
 2. CometChatSnackBoardSegue provides static default view controller sizing based on device.
...
open class CometChatSnackBoardSegue: UIStoryboardSegue {
    ...
    /// The presentation style to use. See the CometChatSnackBoard.PresentationStyle for details.
    public var presentationStyle: CometChatSnackBoard.PresentationStyle {
        get { return messenger.defaultConfig.presentationStyle }
        set { messenger.defaultConfig.presentationStyle = newValue }
    }

    /// The dim mode to use. See the CometChatSnackBoard.DimMode for details.
    public var dimMode: CometChatSnackBoard.DimMode {
        get { return messenger.defaultConfig.dimMode}
        set { messenger.defaultConfig.dimMode = newValue }
    }
    
    // duration
    public var duration: CometChatSnackBoard.Duration {
    ...
    public var eventListeners: [CometChatSnackBoard.EventListener] {
    ...
    */
    ...
    /**
     The view that is passed to `CometChatSnackBoard.show(config:view:)` during presentation.
     The view controller's view is installed into `containerView`, which is itself installed
     into `messageView`. `CometChatSnackBoardSegue` does this installation automatically based on the
    ...
    */
    ...
    private var messenger = CometChatSnackBoard()
    private var selfRetainer: CometChatSnackBoardSegue? = nil
        ...
        messenger.defaultConfig.eventListeners.append { [unowned self] in
            ...
            case .didHide:
                if let completeTransition = self.hider.completeTransition {
                    ...
                    // Case where message is internally hidden by CometChatSnackBoard, such as with a
...
extension CometChatSnackBoardSegue {
    private class TransitioningPresenter: NSObject, UIViewControllerAnimatedTransitioning {
        ...
        private weak var segue: CometChatSnackBoardSegue?

        fileprivate init(segue: CometChatSnackBoardSegue) {
            self.segue = segue
        }
